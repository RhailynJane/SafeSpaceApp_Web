 ...
 * Provides help sections, items, and search functionality
 */
...
export const allSections = query({
  args: {},
  handler: async (ctx) => {
    // Fetch sections ordered by sortOrder then title
    ...
    for (const s of sections) {
      const items = await ctx.db
        .query("helpItems")
        .withIndex("by_sectionId", (q: any) => q.eq("sectionId", s._id))
        .order("asc")
        .collect();
...
export const seedDefault = mutation({
  ...
  handler: async (ctx) => {
    ...
    const sections = [
      { slug: "getting_started", title: "🌟 Getting Started", sortOrder: 1, icon: "🌟", category: "getting_started", priority: "high" },
      { slug: "features", title: "📱 Features Help", sortOrder: 2, icon: "📱", category: "features", priority: "high" },
      { slug: "contact", title: "💬 Contact Support", sortOrder: 6, icon: "💬", category: "support", priority: "low" },
    ];
...
export const _insertSection = internalMutation({
  args: {
    ...
    category: v.optional(v.string()), priority: v.optional(v.string()), sortOrder: v.optional(v.number()),
...
export const _insertItem = internalMutation({
  args: {
    ...
    type: v.optional(v.string()), sortOrder: v.optional(v.number()), related_features: v.optional(v.array(v.string())),
