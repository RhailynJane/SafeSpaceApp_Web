...
/**
 * Resolve the user's role reliably.
 * Uses sessionClaims.publicMetadata.role if present; falls back to Clerk API.
 * @param {string} userId
 * @param {any} sessionClaims
 * @returns {Promise<string|null>} role or null
 */
export async function resolveUserRole(userId, sessionClaims) {
  try {
    ...
    if (!secret) {
      console.warn('CLERK_SECRET_KEY not set; cannot resolve role via Clerk API');
      return null;
    }
    ...
    if (!res.ok) {
      console.warn('Failed to fetch user from Clerk when resolving role');
      return null;
    }
    const user = await res.json();
    return (user?.public_metadata?.role) || null;
  } catch (e) {
    console.warn('resolveUserRole error:', e);
    return null;
  }
}
